...
PKGS_LOCATION='/home/faidoc/Cinnarch/Repos/cinnarch-secinstall2/i686'
...
linux32 pacman -Syy --config pacman.conf.i686
...
for i in `find ${PKGS_LOCATION} -name '*pkg.tar.xz'`;do
	...
	PKG_URL=`linux32 pacman -Sp ${PKG_NAME}`
	PKG_REMOTE_VERSION=`linux32 pacman -Si ${PKG_NAME}|grep Versión|cut -f2 -d ':'`

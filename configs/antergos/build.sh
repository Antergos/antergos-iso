...
make_syslinux() {
    ...
    gzip -c -9 ${work_dir}/root-image/usr/lib/modules/${_uname_r}/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz
}
...
make_customize_root_image() {
	part_one() {
        ...
        part_five() {
        	...
        	mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" \
            	-r 'systemctl -fq enable lightdm plymouth-start NetworkManager ModemManager livecd vboxservice NetworkManager-wait-online ntpd' \
            	run

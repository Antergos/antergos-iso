...
make_customize_root_image() {
	...
	# Copy xz packages
	cache_dirs="/var/cache/pacman/pkg_${arch}"
	cache_dirs_dest="${work_dir}/root-image/var/cache/pacman/pkg"
	mkdir -p ${cache_dirs_dest}
	cp -af ${cache_dirs}/* ${cache_dirs_dest}

	# Get latest Cnchi version from master git
	cnchi_dir="${work_dir}/root-image/usr/share/cnchi"
	rm -rf ${cnchi_dir}
	share_dir="${work_dir}/root-image/usr/share"
	wget "https://github.com/Antergos/Cnchi/archive/master.zip"
	unzip -d ${share_dir} master.zip  
	mv ${share_dir}/Cnchi-master ${share_dir}/cnchi
	rm master.zip
